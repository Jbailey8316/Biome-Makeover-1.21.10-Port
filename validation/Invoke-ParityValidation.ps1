...
$blocks = Sorted (@($parsedBlocks) + @($stage3.blocks) + @($stage3.no_item_blocks) + @($stage4.registry.blocks_with_items) + @($stage4.registry.no_item_blocks) + @($stage5.blocks_with_items) + @($stage5.no_item_blocks) + @($stage6.blocks_with_items) + @($stage6.no_item_blocks) + @($stage8.blocks_with_items) + @($stage8.no_item_blocks) + @($stage9a.blocks_with_items) + @($stage9a.no_item_blocks) + @($stage10c3.blocks_with_items) + @($stage10c3.no_item_blocks))
...
$allNoItemBlocks = @($stage3.no_item_blocks) + @($stage4.registry.no_item_blocks) + @($stage5.no_item_blocks) + @($stage6.no_item_blocks) + @($stage8.no_item_blocks) + @($stage9a.no_item_blocks) + @($stage10c3.no_item_blocks)
...
$sunkenLocation=$sunkenAdvancement.criteria.ruin.conditions.player[0].predicate.location
if($sunkenAdvancement.parent-ne'biomemakeover:biomemakeover/enter_swamp'-or$sunkenAdvancement.display.icon.id-ne'minecraft:cauldron'-or$sunkenAdvancement.display.frame-ne'task'-or$sunkenAdvancement.display.show_toast-ne$true-or$sunkenAdvancement.display.announce_to_chat-ne$true-or$sunkenAdvancement.display.hidden-ne$false-or$sunkenAdvancement.criteria.ruin.trigger-ne'minecraft:location'-or$sunkenLocation.structures.Count-ne1-or$sunkenLocation.structures[0]-ne'biomemakeover:sunken_ruin'-or($sunkenLocation.PSObject.Properties.Name-contains'structure')){Add-Failure 'Stage 10B Sinking Feeling advancement must use the modern plural structures holder list for the canonical structure'}
