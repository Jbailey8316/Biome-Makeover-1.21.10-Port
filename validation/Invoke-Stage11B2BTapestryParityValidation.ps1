...
if ($source -notmatch 'filterBlocks\(' -or $source -notmatch 'Direction filterBlocksFacing = info\.state\(\)\.getValue\(DirectionalBlock\.FACING\)') { throw 'Marker state retrieval is missing' }
...
if ($source -match 'BM_TAPESTRY_(?!TRANSFORM_PROOF)') { throw 'Superseded tapestry forensic logging remains in production source' }
