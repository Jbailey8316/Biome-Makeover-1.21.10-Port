...
$checks = [ordered]@{
  ...
  'game tick delayed timing' = $text.Contains('level.getGameTime() - trace.readyTick') -and $text.Contains('age >= 400') -and $text.Contains('age >= 900')
  'finite D45 disposal' = $text.Contains('BOSS_BOUNDARY_TRACES.remove(trace)')
...
if (-not $SkipJar) {
  ...
  $markers = 'BM_CROP_FINALIZATION_READY','BM_CROP_TARGET_REGISTRATION_MISSING','BM_BOSS_SOURCE_PATH','BM_BOSS_DIRECT_SOURCE','BM_BOSS_SOURCE_PATH_SUMMARY','phase=D20S','phase=D45S'
