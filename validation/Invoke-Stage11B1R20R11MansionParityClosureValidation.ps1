...
Check 'released marker states' ($source.Contains('case "boss" -> level.setBlock(position, Blocks.AIR.defaultBlockState(), 2)') -and $source.Contains('case "arena_pos" -> level.setBlock(position, Blocks.SMOOTH_QUARTZ.defaultBlockState(), 2)'))
...
Check 'no Stage 12 boss activation' (-not $source.Contains('spawnBoss('))
if (-not $SkipJar) {
 ...
 if ($jar) {
  ...
  Check 'marker semantics compiled' ($all.Contains('boss->AIR;arena_pos->SMOOTH_QUARTZ'))
 }
}
