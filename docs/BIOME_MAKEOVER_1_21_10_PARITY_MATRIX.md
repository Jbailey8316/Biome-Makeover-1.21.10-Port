...
| Mushroom House, house-owned Mushroom Trader, and Button Mushrooms disc | Native jigsaw structure/set/pool, original 11x10x11 template at the modern singular `structure/` path, local weighted-pot processor, exact loot, persistent merchant, and native jukebox-song disc | REMEDIATED / AWAITING-STRUCTURE-RUNTIME-RETEST | Initial `/locate` reached a candidate but legacy plural `structures/` made the template manager return an empty template; path corrected and actual 1.21.10 resource/NBT/data-fixer/StructureTemplate load now passes. Runtime: first prove visible house + trader, then resume content checks. |
...
Stage 9 is subdivided by the source-audited dependency graph. Stage 9A, Stage 9B.1 and Stage 9B.2 are complete/runtime accepted. Stage 9B.1 restores the ten data-driven curses and their runtime engine; Stage 9B.2 restores the reachable Altar and `BMCursed` processing loop. Altar waterlogging remains a documented minor parity deviation and is not claimed as functional. Brewing-Stand-style hopper locking is an explicit runtime-accepted Mythas enhancement, not final-source parity. Stage 10A is remediated and awaiting the focused structure runtime retest; Stage 10B and later stages remain unstarted.
