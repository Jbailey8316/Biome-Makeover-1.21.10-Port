...
| Swamp | Runtime accepted; Stage 9A utility complete | Fresh Swamp/Mangrove generation, flora, Decayed behavior, peat, canopy/leaf contracts, pads/lilies, accepted Lightning Bug visuals, waterlogged saplings, and the reachable glass-bottle capture/light block are restored. Peat Composter is complete/runtime accepted in Stage 9A; Sunken Ruins remain later-owned. Final-release-disabled Toad/Tadpole/Wings are excluded. |
...
| Mushroom House, house-owned Mushroom Trader, and Button Mushrooms disc | Native jigsaw structure/set/pool, original 11x10x11 template at the modern singular `structure/` path, local weighted-pot processor, exact loot, persistent merchant, and native jukebox-song disc | COMPATIBLE / RUNTIME PASS | Initial `/locate` reached an empty logical start because legacy plural `structures/` missed the 1.21.10 template loader. After the unchanged NBT moved to singular `structure/`, runtime accepted natural house generation, the embedded and spawned traders, processor pots, chest loot/trades, and non-looping disc playback. Comparator 14 and the advancement toast remain static/package-confirmed rather than separately observed. |

Mushroom Trader natural spawning and Blight Bat activation are disabled in the
final release. The Mushroom Trader itself is active structure-owned content:
the final Mushroom House template embeds one persistent trader, now restored
without re-enabling its commented-out biome spawn hook.
...
| Sunken Ruins, Swamp Jives, secondary Witch Hat | Custom structure/type/piece, nine direct templates, marker/loot path, Swamp Jives, and wearable Witch Hat foundation restored | IMPLEMENTED / AWAITING RUNTIME VALIDATION (STAGE 10B) | Uses singular 1.21.10 structure-template paths, native jukebox-song/equipment systems, and a narrow local wear criterion. Primary Witch death loot/quests remain Stage 12A. |
...
Stage 9 is subdivided by the source-audited dependency graph. Stage 9A, Stage 9B.1 and Stage 9B.2 are complete/runtime accepted. Stage 9B.1 restores the ten data-driven curses and their runtime engine; Stage 9B.2 restores the reachable Altar and `BMCursed` processing loop. Altar waterlogging remains a documented minor parity deviation and is not claimed as functional. Brewing-Stand-style hopper locking is an explicit runtime-accepted Mythas enhancement, not final-source parity. Stage 10A is complete/runtime accepted. Stage 10B is implemented and awaiting runtime validation; Stage 10C and later stages remain unstarted.
