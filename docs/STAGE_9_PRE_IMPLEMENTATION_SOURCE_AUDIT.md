...
Stage 9B audit follow-up: `STAGE_9B_ALTAR_CURSE_PRE_IMPLEMENTATION_AUDIT.md` reconstructs the complete final Altar/ten-curse contract and recommended two checkpoints. Stage 9B.1 and Stage 9B.2 are complete/runtime accepted. `STAGE_9B_2_ALTAR_RESTORATION.md` records the accepted Altar, its deferred nonfunctional waterlogging deviation, and the separately classified Mythas hopper-automation enhancement.
