...
| Mushroom Trader | Green mushroom trader imagery | Separate final registered entity `mushroom_trader`, custom renderer and trade list; its natural biome spawn is commented. Spawn egg is non-hidden, so creative/command reachability remains, but no released survival acquisition path was found in the Mushroom House template/processor. It is not a modified vanilla Wandering Trader. | Entity absent and listed as excluded/unreachable. | HISTORICAL/DISABLED | Do not invent biome trader replacement. Stage 10A may re-verify template NBT at runtime, but current evidence does not make it survival parity. | Final source contains no Wandering Trader mixin or biome-specific vanilla-trader modification. |
