...
public final class CowboyHatLayer<S extends LivingEntityRenderState,M extends EntityModel<? super S>> extends RenderLayer<S,M> {
    ...
    private final boolean equippedPlayerHat;

    public CowboyHatLayer(RenderLayerParent<S,M> parent, net.minecraft.client.model.geom.EntityModelSet models, boolean horse) {
        this(parent,models,horse,false);
    }

    public CowboyHatLayer(RenderLayerParent<S,M> parent, net.minecraft.client.model.geom.EntityModelSet models,
                          boolean horse, boolean equippedPlayerHat) {
        super(parent);
        this.hat=new CowboyHatModel<>(models.bakeLayer(BMModelLayers.COWBOY_HAT));
        this.horse=horse;
        this.equippedPlayerHat=equippedPlayerHat;
    }
    ...
    @Override public void submit(PoseStack pose, SubmitNodeCollector output, int light, S state, float yRot, float xRot) {
        ...
        if(equippedPlayerHat && (!(state instanceof HumanoidRenderState humanoid)
            || !humanoid.headEquipment.is(BMItems.COWBOY_HAT))) return;
        pose.pushPose();
        if(horse) {
