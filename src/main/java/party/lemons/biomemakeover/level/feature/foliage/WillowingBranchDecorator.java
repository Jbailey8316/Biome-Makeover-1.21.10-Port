...
public class WillowingBranchDecorator extends TreeDecorator {
    ...
    protected TreeDecoratorType<?> type() {
        return BMFeatures.WILLOWING_BRANCH_DECORATOR;
    }
        ...
        for(int i = 0; i < 10; i++)
        {
            ...
            for(int j = 0; j < 3; j++)
            {
                if((level.isStateAtPosition(pos, BlockBehaviour.BlockStateBase::isAir) || level.isStateAtPosition(pos, (s)->s == Blocks.WATER.defaultBlockState())) && level.isStateAtPosition(pos.above(), (s)->s.is(BlockTags.LEAVES) || (s.is(BMBlocks.WILLOWING_BRANCHES) && s.getValue(WillowingBranchesBlock.STAGE) < WillowingBranchesBlock.MAX_GROWTH_STAGES)))
                {
                    ...
                    {
                        context.setBlock(pos, BMBlocks.WILLOWING_BRANCHES.defaultBlockState().setValue(WillowingBranchesBlock.STAGE, j).setValue(BlockStateProperties.WATERLOGGED, water));
