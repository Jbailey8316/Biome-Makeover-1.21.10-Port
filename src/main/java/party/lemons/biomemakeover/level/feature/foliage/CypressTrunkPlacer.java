...
public class CypressTrunkPlacer  extends TrunkPlacer {
    ...
    protected TrunkPlacerType<?> type() {
        return BMFeatures.CYPRESS_TRUNK;
    }
    ...
    public List<FoliagePlacer.FoliageAttachment> placeTrunk(LevelSimulatedReader level, BiConsumer<BlockPos, BlockState> biConsumer, RandomSource random, int trunkHeight, BlockPos pos, TreeConfiguration treeConfiguration) {
        ...
        for(int i = 0; i < 2 + random.nextInt(6); i++)
        {
            ...
            BlockPos offsetStart = pos.relative(dir);
    ...
    public static final MapCodec<CypressTrunkPlacer> CODEC = RecordCodecBuilder.mapCodec((instance)->trunkPlacerParts(instance).apply(instance, CypressTrunkPlacer::new));
