...
import party.lemons.biomemakeover.worldgen.mansion.*;
...
public class MansionRoom
{
    ...
    {
        MansionGrid<MansionRoom> lo = layout.getLayout();
        for(int i = 0; i < 4; i++)
        {
    ...
    public void addWalls(MansionDetails details, MansionTemplates templates, RandomSource random, BlockPos wallPos, StructureTemplateManager manager, MansionGrid<MansionRoom> roomGrid, StructurePiecesBuilder children)
    {
        ...
        if(getRoomType().hasWalls())
        {
            ...
            else if(!roomMansionGrid.contains(getPosition().north()) || !roomMansionGrid.get(getPosition().north()).getRoomType().hasWalls())
            ...
            else if(roomMansionGrid.contains(getPosition().north()))
            ...
            else if(!roomMansionGrid.contains(getPosition().west()) || !roomMansionGrid.get(getPosition().west()).getRoomType().hasWalls())
            ...
            else if(roomMansionGrid.contains(getPosition().west()))
            ...
            if(!roomMansionGrid.contains(getPosition().east()) || !roomMansionGrid.get(getPosition().east()).getRoomType().hasWalls())
            ...
            if(!roomMansionGrid.contains(getPosition().south()) || !roomMansionGrid.get(getPosition().south()).getRoomType().hasWalls())
    ...
    public String getOuterWall(MansionTemplates templates, Direction dir, MansionGrid<MansionRoom> roomGrid, RandomSource random)
    {
        ...
        {
            if(getRoomType().hasWindows() && random.nextFloat() < 0.95F && !roomMansionGrid.contains(getPosition().relative(dir)))
    ...
    {
        for(Direction direction : MansionMath.HORIZONTALS)
        {
