...
import party.lemons.biomemakeover.worldgen.mansion.processor.CorridorReplaceProcessor;
import party.lemons.biomemakeover.worldgen.mansion.processor.FloorRoomReplaceProcessor;
import party.lemons.biomemakeover.worldgen.mansion.processor.GardenRoomReplaceProcessor;
import party.lemons.biomemakeover.worldgen.mansion.room.*;
...
public class MansionLayout
{
    ...
    protected final MansionGrid<MansionRoom> layout = new MansionGrid<>();
    ...
    private void createBigRooms(RandomSource random, List<MansionRoom> allRooms)
    {
        for(int i = 0; i < allRooms.size(); i++)
        {
            ...
            {
                for(Direction dir : MansionMath.randomOrderedHorizontals())
                {
    ...
    protected void createDungeon(RandomSource random, List<MansionRoom> allRooms, int startY)
    {
        ...
        Direction bossDir = MansionMath.randomHorizontal(random);  //Direction to boss room
        ...
        for(int i = 0; i < 10; i++)
        {
            ...
            Direction dir;
            do{
                dir = MansionMath.randomHorizontal(random);
    ...
    public MansionGrid<MansionRoom> getLayout()
    {
        return layout;
    }
}